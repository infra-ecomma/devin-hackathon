...
```
User: Make my landing page look like linear.app

Step 1 — Capture inspiration:
  Screenshot of linear.app loaded
  curl + readable extraction of computed CSS

Step 2 — Extract tokens:
  Colors: #0E0F12 bg, #FFFFFF text, #5E6AD2 brand accent, #424348 muted
  Type: Inter (geometric sans), -0.011em letter-spacing on display
  Spacing scale: 4/8/12/16/24/32/48/64 (factor of 4)
  Radii: 6px buttons, 12px cards
  Shadows: subtle drop, layered 0 2px 8px rgba(0,0,0,0.04)

Step 3 — Identify they use Radix UI primitives + custom system
  Tell user: "Linear uses Radix + custom tokens. Recommend adopting Radix
  primitives in your shadcn install; transplant tokens listed above."

Step 4 — Apply to user's tailwind.config.ts:
  Update theme.extend with their token values
  Override neutral palette to match (#0E0F12 → bg-neutral-950)

Step 5 — Component-level adjustments:
  Hero h1: switch to Inter, font-weight 600, tracking -0.011em
  Buttons: rounded-md, py-2 px-3, subtle border instead of fill

Output: tailwind.config.ts diff + 3 component edits
Caveat: inner pages (/pricing) use a different accent — flag for next pass
Chain → design-critique to validate, quality-gate scoring
```
